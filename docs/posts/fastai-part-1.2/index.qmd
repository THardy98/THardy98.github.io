...
same data such that it can learn more generalized patterns.
...
`item_tfms` performs transformations on one *item* at a time, while `batch_tfms`
performs transformations on a *batch* of items at a time. 

18. What is a confusion matrix?

    A *confusion matrix* is a matrix representation of how your model performed
on a classification task. The rows and columns of the matrix are the
classification categories, the row/col category matches on the diagonal. Each
row in the matrix shows how the model predicted for a given category. The
...
19. What does export save?
    ...
    `export` saves the trained model into a file along with its `DataLoaders`
...
25. What is out-of-domain data?
    ...
    *Out-of-domain* data is data that your model was not trained on, thus is out
of the model's *domain* of knowledge.
